...
def findNum(index):
    ...
    if s[index][index]==0:
        answer[index] = 0
        return check and findNum(index+1)

    for i in range(1,11):
        answer[index] = i * s[index][index]
        if check(index) and findNum(index+1):
            return True
    return False
...
s =[]
prev = 0
for i in range(n,0,-1):#배열의 길이
    plus = [0]*(n-i)
    for j in range(prev,prev+i):
        if input_str[j]=="-":
            plus.append(-1)
        elif input_str[j]=="+":
            plus.append(1)
...
# 그때그때 검사가 필요
# 시간복잡도를 줄일 수 있는 방도를 생각할 것
# 여기서 핵심은 s[i][i]는 i의 부호를 나타낸다는 것